...
class Agent:
    ...
    def chase(self):
        # get the nearest human position
        human_list = list(self.humans)
        dist = [(h[0] - 1) ** 2 + (h[1] - 1) ** 2 for h in human_list]
        min_list = [i for i, x in enumerate(dist) if x == min(dist)]
        human_x, human_y = human_list[random.choice(min_list)]
...
# ani.save('agend_living_dead_2.gif', writer="pillow")
plt.show()
